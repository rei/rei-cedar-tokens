import { t as d } from "./cdr-tokens-IdpEO1Fb.js";
const p = {
        ...
        e = n
          ...
          <div class="shadow-card">
            <div class="shadow-stage">
              <div class="shadow-box" style="box-shadow: ${i};"></div>
            </div>
            <div class="shadow-info">
              <span class="shadow-label">${c}</span>
              <span class="shadow-token">${s}</span>
              <span class="shadow-value">${i}</span>
            </div>
          </div>`;
          })
          .join("");
      return `${t}<div class="sb-page">
      <div class="sb-section">
        <div class="sb-section-header">
          <h2 class="sb-section-title">Prominence (Box Shadow)</h2>
          <span class="sb-section-count">${n.length}</span>
        </div>
        <div class="shadow-grid">${e}</div>
      </div>
    </div>`;
    },
  },
  o = {
    ...
    render: () => {
      const a = Object.entries(d)
          .filter(([e]) => e.startsWith("CdrProminence"))
          .map(([e, s]) => [e, String(s)]),
        n = a
          .map(
            ([e, s]) => `
          <div class="prominence-row">
            <span class="prominence-name">${e}</span>
            <div class="prominence-row-inner">
              <div class="prominence-swatch" style="box-shadow: ${s};"></div>
              <span class="prominence-val">${s}</span>
            </div>
          </div>`,
          )
          .join("");
...
const m = ["Prominence", "AllProminence"];
export {
  o as AllProminence,
  r as Prominence,
