...
| Component tokens: `accordion`, `button`, `chip`, `form`, `icon`, `input`, `link`, `message`, `modal`, `pagination`, `rating`, `slide`, `surface`, `surface-selection`, `switch`, `tab`, `table`, `toggle-button`, `tooltip` | B    | Matches Phase 4 of `04-migration-plan.md` ("Component Token Retirement"). Recommend soft-deprecation per-domain as each one's replacement semantic-color mapping is confirmed (see `03-future-state-proposal.md` §4 legacy mapping table), rather than deprecating all 18 at once. |
...
4. Confirm component-token deprecation sequencing: all 18 at once, or phased as each
   domain's semantic-color replacement is confirmed (recommended: phased, tied to
   `04-migration-plan.md` Phase 4's per-domain checklist).
