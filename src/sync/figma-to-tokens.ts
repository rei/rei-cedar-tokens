...
import { tokenFilesFromLocalVariables } from '../tokens-export.js';
...
//   console.log(
//     `Found ${
//       Object.keys(localVariables.meta.variables).length
//     } variables in ${fileKey}`
//   );
...
//     console.log(
//       `No token files generated from variables in Figma file: ${fileKey}`
//     );
//     return 0;
//   }
      ...
      if (localVariables.meta?.variables && Object.keys(localVariables.meta.variables).length > 0) {
        ...
        for (const [fileName, fileContent] of Object.entries(tokensFiles)) {
          try {
            ...
            console.error(`✗ Failed to write ${fileName}. Error: ${error}`);
          }
        }
      } else {
        console.log(`No local variables found in Figma file: ${fileKey}`);
