 ...
 *
 * This filter includes tokens that are that include surface namespace while
 ...
 */
export const componentSurfaceTokens = (sd: typeof StyleDictionary): void => {
  sd.registerFilter({
    name: 'component-surface-tokens',
    filter: (token: Token) =>
      token.path[0] !== 'options' && token.path[0] !== 'theme' && token.path.includes('surface'),
  });
};
