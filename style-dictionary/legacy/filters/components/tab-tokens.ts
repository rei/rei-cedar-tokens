 ...
 *
 * This filter includes tokens that are that include tab namespace while
 ...
 */
export const componentTabTokens = (sd: typeof StyleDictionary): void => {
  sd.registerFilter({
    name: 'component-tab-tokens',
    filter: (token: Token) =>
      token.path[0] !== 'options' && token.path[0] !== 'theme' && token.path.includes('tab'),
  });
};
