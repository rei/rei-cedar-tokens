...
describe('remove-source-tokens filter', () => {
  ...
  it('should include tokens with any first-level path except options and theme', () => {
    expect(removeSourceTokensFilter(createToken(['button', 'color']))).toBe(true);
    expect(removeSourceTokensFilter(createToken(['spacing', 'small']))).toBe(true);
    expect(removeSourceTokensFilter(createToken(['custom', 'token']))).toBe(true);
  });
