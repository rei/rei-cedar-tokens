 ...
 *
 * This filter includes tokens that are that include chip namespace while
 ...
 */
export const componentChipTokens = (sd: typeof StyleDictionary): void => {
  sd.registerFilter({
    name: 'component-chip-tokens',
    filter: (token: Token) =>
      token.path[0] !== 'options' && token.path[0] !== 'theme' && token.path.includes('chip'),
  });
};
