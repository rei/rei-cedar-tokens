 ...
 *
 * This filter includes tokens that are that include tooltip namespace while
 ...
 */
export const componentTooltipTokens = (sd: typeof StyleDictionary): void => {
  sd.registerFilter({
    name: "component-tooltip-tokens",
    filter: (token: Token) =>
      token.path[0] !== "options" &&
      token.path[0] !== "theme" &&
      token.path.includes("tooltip"),
  });
};
