 ...
 *
 * This filter includes tokens that are that include modal namespace while
 ...
 */
export const componentModalTokens = (sd: typeof StyleDictionary): void => {
  sd.registerFilter({
    name: "component-modal-tokens",
    filter: (token: Token) =>
      token.path[0] !== "options" &&
      token.path[0] !== "theme" &&
      token.path.includes("modal"),
  });
};
