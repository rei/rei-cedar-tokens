import type StyleDictionary from 'style-dictionary';
import type { Token } from 'style-dictionary';

/**
 * Extended Token interface that includes spacingModifier for space calculations.
 */
interface SpaceToken extends Token {
  spacingModifier?: number;
}

/**
 * Registers a transform for spacing tokens with modifiers for JavaScript output.
 *
 * This transform applies a spacing modifier to dimension tokens, outputting the
 * result as a plain numeric string suitable for JavaScript/TypeScript usage.
 * The modifier is multiplied by the base value to calculate the final spacing.
 *
 * Only applies to dimension tokens that have a spacingModifier attribute.
 *
 * @param sd - The Style Dictionary instance to register the transform with
 */
export const spaceJs = (sd: typeof StyleDictionary): void => {
  sd.registerTransform({
    name: 'size/space-js',
    type: 'value',
    transitive: true,
    filter: (token: Token): boolean =>
      token.$type === 'dimension' && !!(token as SpaceToken).spacingModifier,
    transform: (token: Token): string => {
      const { $value, spacingModifier } = token as SpaceToken;
      const match = $value.match(/^(-?[\d.]+)\s*(.*)$/);
      if (!match) {
        const num = parseFloat($value) * (spacingModifier || 1);
        return String(parseFloat(num.toFixed(10)));
      }
      const rawValue = parseFloat(match[1]);
      const unit = match[2];
      const num = rawValue * (spacingModifier || 1);
      const result = parseFloat(num.toFixed(10));
      return unit ? `${result}${unit}` : String(result);
    },
  });
};
