...
describe("utils", () => {
  ...
  describe("filterSourceTokensAndType", () => {
    ...
    it("should handle array of types", () => {
      ...
      const dimensionToken = createToken(
      ...
      expect(
        filterSourceTokensAndType(colorToken, ["color", "dimension"]),
      ).toBe(true);
      expect(
        filterSourceTokensAndType(dimensionToken, ["color", "dimension"]),
      ).toBe(true);
      expect(filterSourceTokensAndType(fontToken, ["color", "dimension"])).toBe(
        false,
      );
    });
